...
";
	 ...
	 * @author	Davey IJzermans
	 */
	public function notify($message, $user_id, $type = "info", $location = 'html,js', $force = false) {
		if (is_array($message)) {
			...
			$this->notifications[] = $this->make_notification($message, $user_id, $type, $location, $force);
	 ...
	 * @author	Davey IJzermans
	 */
	public function make_notification($message, $user_id, $obj_id, $type = "info", $location = 'html,js', $force = false) {
		$notification = array(
			'message' => $message,
                        'user_id'=>$user_id,
                        'obj_id'=>$obj_id,
			...
			'force' => $force
                ...
                $this->insert_notification($insert);
		return $notification;
	}
	function insert_notification($params) {
            $this->CI->db->insert('notifications',$params);
        }
        function get_notifications($user_id) {
            $sql = "SELECT * FROM notifications WHERE user_id=$user_id ORDER by date desc";
        ...
        function get_notifications_count($user_id) {
            $sql = "SELECT COUNT(*) as t FROM notifications WHERE user_id=$user_id AND new=1";
            $result = $this->CI->db->query($sql);
            if($result->num_rows()>0) {
                return $result->row()->t;
            }
	...
	public function clear() {
		...
		//return $this->update_session_data();
                $update = array('new'=>0);
                $this->CI->db->update('notifications',$update);
                $this->CI->db->where('user_id',get_logged_in_user_id());
